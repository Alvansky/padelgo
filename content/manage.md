...
function adminRenderBookingsTable(bookings) {
  ...
  bookings.forEach(function(b) {
    ...
    html += '<tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50"><td class="px-4 py-3 text-sm">' + adminEsc(name) + '</td><td class="px-4 py-3 text-sm font-bold">' + adminEsc(b.court_name || b.court_id) + '</td><td class="px-4 py-3 text-sm">' + b.date + '</td><td class="px-4 py-3 text-sm">' + b.start_time + ' - ' + b.end_time + '</td><td class="px-4 py-3"><span class="rounded-full px-2.5 py-0.5 text-xs font-bold ' + adminStatusClass(b.status) + '">' + adminStatusLabel(b.status) + '</span></td><td class="px-4 py-3 text-sm font-bold">' + adminRupiah(b.amount) + '</td><td class="px-4 py-3">' + actions + '</td></tr>';
  });
...
function adminRenderCourts() {
  ...
  adminData.courts.forEach(function(c) {
    html += '<div class="flex items-center justify-between px-6 py-4"><div><p class="font-bold text-slate-900 dark:text-white">' + adminEsc(c.name) + '</p><p class="text-sm text-slate-500">' + adminRupiah(c.price_per_hour) + '/jam</p></div><span class="rounded-full px-2.5 py-0.5 text-xs font-bold ' + (c.available ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700') + '">' + (c.available ? 'Aktif' : 'Hidden') + '</span></div>';
  });
...
function adminStatusClass(s) { return s === 'approved' ? 'bg-emerald-100 text-emerald-700' : s === 'pending' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'; }
